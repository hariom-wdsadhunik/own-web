  ...
  return (
    <section id="about" className="py-16 sm:py-24 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-start">
        <div className="lg:col-span-7 space-y-4 sm:space-y-6">
          {ABOUT_INFO.bio.map((paragraph, i) => (
            <FadeIn key={i} delay={i * 0.1}>
              <p className="font-sans text-sm sm:text-lg text-gray-300 leading-relaxed font-normal">
                {paragraph}
              </p>
            </FadeIn>
          ))}
            ...
            <div className="pt-4 sm:pt-6 border-t border-white/10 space-y-3">
              ...
              <div className="flex flex-wrap gap-2">
                {ABOUT_INFO.focus.map((item) => (
                  <Badge key={item} variant="icy" className="text-[10px] sm:text-xs">
                    {item}
                  </Badge>
                ))}
              </div>
        ...
        <div className="lg:col-span-5 space-y-5 bg-[#0f1117] p-6 sm:p-8 rounded-xl border border-white/10">
          <span className="font-mono text-xs text-blue-400 uppercase tracking-widest block border-b border-white/10 pb-4">
            Operating Principles
          </span>

          <ul className="space-y-3 sm:space-y-4 font-mono text-xs text-gray-300">
